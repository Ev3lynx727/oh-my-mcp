...
Metrics are auto-generated by `prom-client` (see `src/infrastructure/metrics/metrics.ts` for definitions).
...
- For stdio transport servers, the gateway forwards JSON-RPC via stdin/stdout directly.
- For supergateway transport servers, the gateway returns **501** — clients connect to the server's SSE endpoint directly.
- Server-sent events (SSE) are supported by supergateway; clients subscribe via the server's `/sse` endpoint.
...
SSE streaming is supported through supergateway — clients connect to each server's `/sse` endpoint directly (e.g., `http://localhost:8101/sse`). The gateway operates in request/response mode for stdio transport. WebSocket streaming is on the roadmap.
